...
def task_generate(model, data, config, evaluator, num_batch, dest_f=None, verbose=True):
    ...
    task_report_new, _s, _m= evaluator.evaluateModel(generated_dialogs, mode=data.name, new_version=True)
    ...
    resp_report += 'total score {:.4f}\n'.format(remix)
    logger.info(resp_report)
    logger.info('Generation Done')
    # write(resp_report)
    # write('Generation Done')
    return success, match, bleu, f1, remix
