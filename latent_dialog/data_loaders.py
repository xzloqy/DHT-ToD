...
import json
from gbn_woz.utils import Bag_of_words, get_doc_sent, Bow_sents
...
class BeliefDbDataLoaders(BaseDataLoaders):
    def __init__(self, name, data, V_tm, tmvocab_ignore, config):
        super(BeliefDbDataLoaders, self).__init__(name)
        self.max_utt_len = config.max_utt_len
        self.data, self.indexes, self.batch_indexes = self.flatten_dialog(data, config.backward_size, V_tm, tmvocab_ignore)
        self.data_size = len(self.data)
        self.domains = ['hotel', 'restaurant', 'train', 'attraction', 'hospital', 'police', 'taxi']

    def flatten_dialog(self, data, backward_size, V_tm, tmvocab_ignore):
        results = []
        indexes = []
        batch_indexes = []
        resp_set = set()
        ...
        if len(data) < 2000:
            for dlg in data:
                ...
                batch_index = []
                goal_num += 1
                sent_all = [0.0]*(V_tm-len(tmvocab_ignore))
                for i in range(1, len(dlg.dlg)):
                    if dlg.dlg[i].speaker == USR:
                        continue
                    e_idx = i
                    ...
                    sent_T = sent_all
                    sent_all = sent_all + Bow_sents(response.utt, V_tm, tmvocab_ignore)
                    context = []
                    for turn in dlg.dlg[s_idx: e_idx]:
                        turn['utt'] = self.pad_to(self.max_utt_len, turn.utt, do_pad=False)
                        context.append(turn)
                    results.append(Pack(context=context, response=response, goal=goal, key=key, sent_T=sent_T))
        ...
        # train set
            ...
            for dlg in data:
                ...
                for i in range(1, len(dlg.dlg)):
                    ...
                    response['utt'] = self.pad_to(self.max_utt_len, response.utt, do_pad=False)
                    resp_set.add(json.dumps(response.utt))
                    sent_T = sent_all - Bow_sents(response.utt, V_tm, tmvocab_ignore)
                    context = []
                    for turn in dlg.dlg[s_idx: e_idx]:
                        turn['utt'] = self.pad_to(self.max_utt_len, turn.utt, do_pad=False)
                        context.append(turn)
    ...
    def _prepare_batch(self, selected_index):
        ...
        for row in rows:
            in_row, out_row, goal_row, sent_T_row = row.context, row.response, row.goal, row.sent_T
            ...
            # topic context
            sent_T = [t for idx, t in enumerate(sent_T_row)]
            sents.append(sent_T)
