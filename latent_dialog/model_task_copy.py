...
class GBN2Gauss(BaseModel):
    def __init__(self, corpus, config):
        ...
        if self.gbn:
            self.c2z = nn_lib.Hidden2Gaussian(self.utt_encoder.output_size + self.db_size + self.bs_size + 230,
                                          config.y_size, is_lstm=False)
        else:
            self.c2z = nn_lib.Hidden2Gaussian(self.utt_encoder.output_size + self.db_size + self.bs_size,
                                          config.y_size, is_lstm=False)
        ...
        self.state1 = nn.Sequential(nn.Linear(4700, 100),
                                    nn.Sigmoid(),)
        self.state2 = nn.Sequential(nn.Linear(100, 80),
                                    nn.Sigmoid(),)
        self.state3 = nn.Sequential(nn.Linear(80, 50),
                                    nn.Sigmoid(),)
    ...
    def KL_GamWei(self, Gam_shape, Gam_scale, Wei_shape, Wei_scale):  # K_dim[i] * none
        eulergamma = 0.5772
        KL_Part1 = eulergamma * (1 - 1 / Wei_shape) + self.log_max_tf(Wei_scale / Wei_shape) + 1 + Gam_shape * Gam_scale
    ...
    def topic_driven(self, topic_context, doc_num_batches, MBObserved):
        ...
        for j in range(sent_J):
            ...
            theta1_KL = self.KL_GamWei(alpha_1_t, np.float32(1.0), self.k_1, self.l_1).sum()
            theta2_KL = self.KL_GamWei(alpha_2_t, np.float32(1.0), self.k_2, self.l_2).sum()
            theta3_KL = self.KL_GamWei(alpha_3_t, np.float32(1.0), self.k_3, self.l_3).sum()
    ...
    def forward(self, data_feed, mode, doc_num_batches=0, MBObserved=0, clf=False, gen_type='greedy', use_py=None, return_latent=False):
        ...
        if self.gbn:
            ...
            self.topic_driven(topic_contextss, doc_num_batches, MBObserved)
        ...
        if self.gbn:
            enc_last = th.cat([bs_label, db_label, utt_summary.squeeze(1),self.HT], dim=1)
            # enc_last = self.gbn_latent(enc_last)
        else:
            enc_last = th.cat([bs_label, db_label, utt_summary.squeeze(1)], dim=1)
        # create decoder initial states
        if self.simple_posterior:
