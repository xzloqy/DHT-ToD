...
class MultiWozDB_2(object):
    ...
    for domain in domains:
        db = os.path.join(CUR_DIR, 'data/multiwoz_2.1/db/{}-dbase.db'.format(domain))
        conn = sqlite3.connect(db)
        c = conn.cursor()
        dbs[domain] = c
...
class MultiWozEvaluator(BaseEvaluator):
    ...
    def __init__(self, data_name='norm-multi-woz'):
        if data_name == 'multiwoz_2.1':
            self.data_name = data_name
            self.db = MultiWozDB_2()

        else:
            self.data_name = 'norm-multi-woz'
            self.db = MultiWozDB()

        self.slot_dict = delex.prepareSlotValuesIndependent(self.data_name)
        ...
        self.delex_dialogues = json.load(open(os.path.join(self.CUR_DIR, 'data', self.data_name, 'delex.json')))
    ...
    def evaluateModel(self, dialogues, mode='valid', new_version=False):
        ...
        report = ""
        report += '{} Corpus Matches({}) : {:2.2f}%, Groundtruth {} Matches : {:2.2f}%'.format(mode, new_version, (matches / float(total) * 100), mode, (corpus_matches / float(total) * 100)) + "\n"
        report += '{} Corpus Success({}) : {:2.2f}%, Groundtruth {} Success : {:2.2f}%'.format(mode, new_version, (successes / float(total) * 100), mode, (corpus_successes / float(total) * 100)) + "\n"
        report += 'Total number of dialogues: %s, new version=%s ' % (total, new_version)
        if not mode=='offline_rl':
