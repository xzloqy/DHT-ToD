...
class DecoderRNN(BaseRNN):
    def __init__(self, input_dropout_p, rnn_cell, input_size, hidden_size, num_layers, output_dropout_p,
        ...
        # gbn
        # h_size = 300
        # self.h2zr = nn.Linear(230+h_size,h_size*2)
        # self.t2c = nn.Linear(230,h_size,bias=False)
        # self.h2c = nn.Linear(h_size,h_size)

    def forward(self, batch_size, dec_inputs, dec_init_state, attn_context, mode, gen_type, beam_size, topic_hid=None, goal_hid=None):
        ...
        if mode == TEACH_FORCE:
            prob_outputs, decoder_hidden_state, attn = self.forward_step(input_var=decoder_input, hidden_state=decoder_hidden_state, encoder_outputs=attn_context, goal_hid=goal_hid, topic_hid=topic_hid)
        else:
            ...
            for step in range(self.max_dec_len):
                ...
                decoder_output, decoder_hidden_state, step_attn = self.forward_step(decoder_input, decoder_hidden_state, attn_context, goal_hid=goal_hid, topic_hid=topic_hid)
    ...
    # def gate(self, hidden, theta):
    #     h_size = hidden.size(-1)
    #     z, r = th.split(self.h2zr(th.cat([hidden,theta],dim=-1)),h_size,dim=-1)
    #     z, r = th.sigmoid(z), th.sigmoid(r)
    #     c = th.tanh(self.t2c(theta) + self.h2c(r * hidden))
    #     hidden = (1-z)*hidden + z*c
    #     return hidden

    def forward_step(self, input_var, hidden_state, encoder_outputs, goal_hid, topic_hid):
        ...
        # output: (batch_size, output_seq_len, dec_cell_size)
        # hidden: tuple: (h, c)
        output, hidden_s = self.rnn(embedded, hidden_state) # 32*32*300      2*  1*32*300
        # if topic_hid is not None:
        #     topic_hid = topic_hid.repeat(1,output_seq_len).reshape(batch_size*output_seq_len,-1)
        #     output = output.reshape(batch_size*output_seq_len,-1)
        #     output = self.gate(output, topic_hid)
        attn = None
        if self.use_attn:
            # output: (batch_size, output_seq_len, dec_cell_size)
            # encoder_outputs: (batch_size, max_ctx_len, ctx_cell_size)
