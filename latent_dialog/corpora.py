...
import logging
from gbn_woz.utils import gen_vocab
from gbn_woz.utils import Bag_of_words, get_doc_sent
...
class NormMultiWozCorpus(object):
    ...
    def _process_dialogue(self, data):
        ...
        for key, raw_dlg in data.items():
            # TODO use target domain
            # if not len(raw_dlg['goal'][self.tgt_domain]):
            #     continue
    ...
    def _extract_vocab(self):
        ...
        self.vocab_dict = {t: idx for idx, t in enumerate(self.vocab)}
        # topic words 
        stopwords = '../gbn_woz/en.txt'
        if 'ignore_delx' in self.config and self.config.ignore_delx == False:
            self.ignore_delx = False
        else:
            self.ignore_delx = True
        self.tmvocab_ignore = gen_vocab(SPECIAL_TOKENS,self.vocab_dict,stopwords,self.ignore_delx)
        self.V_tm = len(self.vocab_dict) - len(self.tmvocab_ignore)
        self.unk_id = self.vocab_dict[UNK]
        self.logger.info("Raw vocab size {} in train set and final vocab size {}".format(raw_vocab_size, len(self.vocab)))
        self.tm_dict = []
        for idx, t in enumerate(self.vocab):
            if idx not in self.tmvocab_ignore:
                self.tm_dict.append(t)
    ...
    def get_corpus(self):
        ...
        id_val = self._to_id_corpus('Valid', self.val_corpus)
        id_test = self._to_id_corpus('Test', self.test_corpus)
        # topic_full = id_train + id_val + id_test
        return id_train, id_val, id_test
    ...
    def _to_id_corpus(self, name, data):
        ...
        for dlg in data:
            ...
            # tm_sent = []
            # for turn in dlg.dlg:
            #     for utts in turn.utt:
            #         if utts in self.vocab_dict.keys() and self.vocab_dict[utts] not in self.tmvocab_ignore:
            #             tm_sent.append(self.vocab_dict[utts])
            for turn in dlg.dlg:
                id_turn = Pack(utt=self._sent2id(turn.utt),
                               speaker=turn.speaker,
                ...
                id_dlg.append(id_turn)
            id_goal = self._goal2id(dlg.goal)
            # results.append(Pack(dlg=id_dlg, goal=id_goal, key=dlg.key, tm_sent=tm_sent))
