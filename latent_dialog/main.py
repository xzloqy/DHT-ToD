from gbn_woz.utils import update_Pi_Phi
import os
import sys
import numpy as np
import torch as th
...
from latent_dialog import evaluators_new as evaluators
from latent_dialog.record import record, record_task, UniquenessSentMetric, UniquenessWordMetric
import logging
import random
from gbn_woz.dialog_utils import dump_latent

logger = logging.getLogger()
def extent_topic_dict(topic_dict, Phi):
    ...
    for i in range(len(phi_0)):
        theset = frozenset(phi_0[i])
        theset = sorted(theset, reverse=True)
        thedict = {}
        out_list = []
        for j in range(5):
            positions = [k for k, x in enumerate(phi_0[i]) if x == theset[j]]
            out_list.append(topic_dict[positions[0]])
    ...
    for i in range(len(phi_1)):
        theset = frozenset(phi_1[i])
        theset = sorted(theset, reverse=True)
        thedict = {}
        out_list = []
        for j in range(5):
            positions = [k for k, x in enumerate(phi_1[i]) if x == theset[j]]
            out_list.append(str(positions[0]))
            # print('1st = ' + str(theset[0]) + ' at ' + str(thedict.get(theset[0])))
            # print('2nd = ' + str(theset[1]) + ' at ' + str(thedict.get(theset[1])))
            # print('3rd = ' + str(theset[2]) + ' at ' + str(thedict.get(theset[2])))
        result_list2.append(out_list)
    for i in range(len(phi_2)):
        theset = frozenset(phi_2[i])
        theset = sorted(theset, reverse=True)
        thedict = {}
        out_list = []
        for j in range(5):
            positions = [k for k, x in enumerate(phi_2[i]) if x == theset[j]]
            out_list.append(str(positions[0]))
            # print('1st = ' + str(theset[0]) + ' at ' + str(thedict.get(theset[0])))
            # print('2nd = ' + str(theset[1]) + ' at ' + str(thedict.get(theset[1])))
            # print('3rd = ' + str(theset[2]) + ' at ' + str(thedict.get(theset[2])))
        result_list3.append(out_list)
    a = phi_0[32]
    final = [27,40,24,31,49,9,45,16]
    out_1 = []
    for i in final:
        a = a + phi_0[i]
    theset = frozenset(a)
    ...
    out_list = []
    for j in range(5):
        positions = [k for k, x in enumerate(a) if x == theset[j]]
        out_1.append(topic_dict[positions[0]])
    ...
    for j in range(5):
        positions = [k for k, x in enumerate(a) if x == theset[j]]
        out_21.append(topic_dict[positions[0]])
    ...
    for j in range(5):
        positions = [k for k, x in enumerate(a) if x == theset[j]]
        out_22.append(topic_dict[positions[0]])   
    ...
    for j in range(5):
        positions = [k for k, x in enumerate(a) if x == theset[j]]
        out_23.append(topic_dict[positions[0]])
...
class Reinforce(object):
    ...
    def run(self):
        ...
        # load self.phi pi
        state = th.load(self.rl_config.reward_best_model_path)
        self.sys_model.load_state_dict(state['model'])
        self.sys_model.gbn_topic.Phi = state['phi']
        self.sys_model.gbn_topic.Pi = state['pi']
...
class OfflineTaskReinforce(object):
    def __init__(self, agent, corpus, sv_config, sys_model, rl_config, generate_func):
        ...
        self.train_data = BeliefDbDataLoaders('Train', train_dial, V, corpus.tmvocab_ignore, self.sv_config)
        self.sl_train_data = BeliefDbDataLoaders('Train', train_dial, V, corpus.tmvocab_ignore, self.sv_config)
        self.val_data = BeliefDbDataLoaders('Val', val_dial, V, corpus.tmvocab_ignore, self.sv_config)
        self.test_data = BeliefDbDataLoaders('Test', test_dial, V, corpus.tmvocab_ignore, self.sv_config)
        ...
        # evaluation
        data_name = 'SYS_WOZ'
        if 'data_name' in sv_config:
            if sv_config.data_name == 'multiwoz_2.1':
                data_name = sv_config.data_name
        self.evaluator = evaluators.MultiWozEvaluator(data_name)
    ...
    def run(self):
        ...
        t_success, t_match, t_bleu, t_f1, t_remix = self.generate_func(self.sys_model, self.test_data, self.sv_config,
                                                              self.evaluator, None, verbose=False)

        self.ppl_test_file.write('{}\t{}\t{}\t{}\t{}\n'.format(n, np.exp(test_loss), t_bleu, t_f1, t_remix))
        self.ppl_test_file.flush()

        self.rl_test_file.write('{}\t{}\t{}\t{}\t{}\n'.format(n, (t_success + t_match), t_success, t_match, t_remix))
        ...
        try:
            for epoch_id in range(self.rl_config.nepoch):
                ...
                while True:
                    ...
                    n += 1
                    if n % 50 == 0:
                        logger.info("Reinforcement Learning {}/{} eposide".format(n, self.train_data.num_batch*self.rl_config.nepoch))
                    ...
                    if self.rl_config.record_freq > 0 and n % self.rl_config.record_freq == 0:
                        ...
                        v_success, v_match, v_bleu, v_f1, v_remix = self.generate_func(self.sys_model, self.val_data, self.sv_config, self.evaluator, None, verbose=False)
                        self.ppl_val_file.write('{}\t{}\t{}\t{}\t{}\n'.format(n, np.exp(valid_loss), v_bleu, v_f1, v_remix))
                        self.ppl_val_file.flush()
                        self.rl_val_file.write('{}\t{}\t{}\t{}\t{}\n'.format(n, (v_success + v_match), v_success, v_match, v_remix))
                        self.rl_val_file.flush()

                        test_loss = self.validate_func(self.sys_model, self.test_data, self.sv_config, use_py=True)
                        t_success, t_match, t_bleu, t_f1, t_remix = self.generate_func(self.sys_model, self.test_data, self.sv_config, self.evaluator, None, verbose=False)
                        self.ppl_test_file.write('{}\t{}\t{}\t{}\t{}\n'.format(n, np.exp(test_loss), t_bleu, t_f1, t_remix))
                        self.ppl_test_file.flush()
                        self.rl_test_file.write('{}\t{}\t{}\t{}\t{}\n'.format(n, (t_success + t_match), t_success, t_match, t_remix))
                        self.rl_test_file.flush()
                        ...
                        if v_success+v_match > best_rewards:
                            ...
                            if not self.sys_model.gbn:
                                th.save(self.sys_model.state_dict(), self.rl_config.reward_best_model_path)
                            # save self.phi pi
                            else:
                                state = {'model':self.sys_model.state_dict(), 'phi':self.sys_model.gbn_topic.Phi, 'pi':self.sys_model.gbn_topic.Pi}
                                th.save(state, self.rl_config.reward_best_model_path)
    ...
    def test(self):
        logger.info("$$$ Load {}-model".format(self.rl_config.rl_best_model))
        # self.sv_config.batch_size = 32
        if not self.sv_config.gbn:
            self.sys_model.load_state_dict(th.load(self.rl_config.rl_best_model))
        # load self.phi pi
        else:
            state = th.load(self.rl_config.rl_best_model)
            self.sys_model.load_state_dict(state['model'])
            self.sys_model.gbn_topic.Phi = state['phi']
            self.sys_model.gbn_topic.Pi = state['pi']

        #for topic word export
        # Phi = state['phi']
        # Pi = state['pi']
        # tm_dict = self.corpus.tm_dict
        # extent_topic_dict(tm_dict,Phi)
        validate(self.sys_model, self.val_data, self.sv_config, use_py=True)
        validate(self.sys_model, self.test_data, self.sv_config, use_py=True)
        latent_results = dump_latent(self.sys_model, self.test_data, self.sv_config)
    ...
    def export_topic(self):
        if not self.sv_config.gbn:
            print("not in gbn mode!")
            return 0
        else:
            Phi = self.sys_model.gbn_topic.Phi
            Pi = self.sys_model.gbn_topic.Pi
            tm_dict = self.corpus.tm_dict
            topic_list = extent_topic_dict(tm_dict,Phi)
            return topic_list
...
def train(model, train_data, val_data, test_data, config, evaluator, gen=None):
    ...
    #gbn
    # doc_num_batches = int(np.floor(float(len(train_Doc)) / 32))
    # batch_ids = [item for item in range(doc_num_batches)]
    doc_num_batches = int(np.floor(float(len(train_data.data)) / config.batch_size))
    while True:
        ...
        if 'ckpt_step' in config:
            ...
            ckpt_step = train_data.num_batch
        while True:
            batch = train_data.next_batch()
            # num_batch = train_data.num_batch
            # random.shuffle(batch_ids)
            MBObserved = batch_cnt
            # Doc = train_Doc[(batch_ids[batch_cnt] * 32):((batch_ids[batch_cnt] + 1) * 32)]
            # X_train_batch, y_train_batch,d_train_batch, m_train_batch = get_batch_tm_lm_all(Doc, train_doc_bow, len(idxvocab), tm_ignore, 32)
            if batch is None:
                break
    
            optimizer.zero_grad()
            loss = model(batch, mode=TEACH_FORCE, doc_num_batches=doc_num_batches, MBObserved=MBObserved)
            model.backward(loss, batch_cnt)
            nn.utils.clip_grad_norm_(model.parameters(), config.grad_clip)
            optimizer.step()
            batch_cnt += 1
            ...
            if batch_cnt % config.print_step == 0:
                ...
                logger.info(train_loss.pprint('Train',
                                        ...
                                        prefix='{}/{}-({:.3f})'.format(batch_cnt%ckpt_step, ckpt_step, model.kl_w)))
                sys.stdout.flush()
    
            if batch_cnt % ckpt_step == 0:
                ...
                # generation
                if gen is not None:
                    success, match, bleu, f1, remix = gen(model, val_data, config, evaluator, num_batch=config.preview_batch_num)
                ...
                if remix > best_remix:
                    ...
                    if config.save_model:
                        ...
                        # save self.phi pi
                            ...
                            state = {'model':model.state_dict(), 'phi':model.gbn_topic.Phi, 'pi':model.gbn_topic.Pi}
                            th.save(state, os.path.join(config.saved_path, '{}-model'.format(done_epoch)))
                        best_epoch = done_epoch
                        saved_models.append(done_epoch)
                        if len(saved_models) > last_n_model:
                            remove_model = saved_models[0]
...
# def draw(model, val_data, config, batch_cnt=None, use_py=None):
#     model.eval()
#     val_data.epoch_init(config, shuffle=False, verbose=False)
#     while True:
#         batch = val_data.next_batch()
...
#             break
#         if use_py is not None:
#             ret_dict, labels = model(batch, mode=TEACH_FORCE, use_py=use_py)
#         else:
#             ret_dict, labels = model(batch, mode=TEACH_FORCE)
