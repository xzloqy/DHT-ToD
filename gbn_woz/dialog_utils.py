...
def dump_latent(model, data, config):
    ...
    while True:
        ...
        for b_id in range(pred_labels.shape[0]):
            ...
            draw_point["sample_y"].append(sample_y[b_id])
            draw_point["domain"].append(batch['goals_list'])
            draw_point["true_str"].append(true_str)
            draw_point["pred_str"].append(pred_str)
