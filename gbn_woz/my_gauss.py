...
config = Pack(
    ...
    init_range=0.1,
    pretrain_folder='gbn_gauss_best',
    forward_only=False
)
set_seed(config.seed)
start_time = time.strftime('%Y-%m-%d-%H-%M-%S', time.localtime(time.time()))
stats_path = 'gauss_model'
# stats_path = 'best_result'
if config.forward_only:
    ...
    if config.use_gbn == True:
        prefix = 'gbn_gauss' + str(config.batch_size) + "_w" + str(config.max_vocab_size)
        prefix = prefix + '_' + str(config.ignore_delx) + "_T=" + str(cf.K)
    else:
        prefix = 'base_gauss' + str(config.batch_size)
